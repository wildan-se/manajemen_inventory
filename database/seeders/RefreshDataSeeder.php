 ...
 * RefreshDataSeeder
 *
 * Truncates all tables EXCEPT users & personal_access_tokens,
 * then re-seeds with fresh master + transactional data.
 *
 * Usage:
 *   php artisan db:seed --class=RefreshDataSeeder
 */
class RefreshDataSeeder extends Seeder
{
  public function run(): void
  {
    ...
    // ════════════════════════════════════════════════════════════
    //  TRUNCATE (semua kecuali users & personal_access_tokens)
    // ════════════════════════════════════════════════════════════
    ...
    //  LOAD EXISTING USERS
    // ════════════════════════════════════════════════════════════
    $admin  = User::where('email', 'admin@skbu.com')->first();
    $inv    = User::where('email', 'inventory@skbu.com')->first();
    $whop   = User::where('email', 'gudang@skbu.com')->first();
    $spv    = User::where('email', 'supervisor@skbu.com')->first();
    $prod   = User::where('email', 'produksi@skbu.com')->first();

    if (! $admin || ! $inv || ! $whop || ! $spv || ! $prod) {
      $this->command->error('One or more required users not found. Run migrate:fresh --seed first.');
      return;
    }
    ...
    Stock::create(['item_id' => $i10->id, 'warehouse_id' => $whBB->id,   'quantity' => 14]);
    ...
    Stock::create(['item_id' => $i15->id, 'warehouse_id' => $whBJ->id,   'quantity' => 4]);
    Stock::create(['item_id' => $i16->id, 'warehouse_id' => $whBJ->id,   'quantity' => 2]);
    ...
    // Goods receipt partial PO-4
    $d = Carbon::now()->subWeeks(2);
    StockMovement::create(['reference_number' => $smRef(), 'type' => 'goods_receipt', 'item_id' => $i4->id, 'to_warehouse_id' => $whBB->id, 'quantity' => 60,  'quantity_before' => 4,   'quantity_after' => 64,  'reference_document' => 'PO-202601-0002', 'notes' => 'Penerimaan tahap 1 — elektroda las', 'user_id' => $whop->id, 'created_at' => $d, 'updated_at' => $d]);
    StockMovement::create(['reference_number' => $smRef(), 'type' => 'goods_receipt', 'item_id' => $i3->id, 'to_warehouse_id' => $whBB->id, 'quantity' => 150, 'quantity_before' => 336, 'quantity_after' => 486, 'reference_document' => 'PO-202601-0002', 'notes' => 'Penerimaan tahap 1 — pelat besi', 'user_id' => $whop->id, 'created_at' => $d, 'updated_at' => $d]);

    // Material issues WO-3
    ...
    StockOpnameItem::create(['stock_opname_id' => $op1->id, 'item_id' => $i2->id, 'system_quantity' => 200,  'physical_quantity' => 198,  'discrepancy' => -2,  'notes' => '2m besi hollow patah, dibuang']);
