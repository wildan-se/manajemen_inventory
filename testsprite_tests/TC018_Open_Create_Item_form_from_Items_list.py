...
async def run_test():
    ...
    try:
        ...
        # -> Type 'admin@mjmetal.co.id' into the email field (index 6).
        ...
        # -> Click the 'Items / Material' menu link (index 190) to open the Items list page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/aside/nav/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the '+ Tambah Item' button (index 965) to open the Create Item page.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        frame = context.pages[-1]
        # Verify URL contains "/items/create"
        assert "/items/create" in frame.url
